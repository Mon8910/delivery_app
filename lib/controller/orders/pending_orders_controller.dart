...
import 'package:delivery_apps/data/datasource/orders/pending_orders_data.dart';
...
class PendingOrdersController extends GetxController {
  ...
  approveOrders(userid,ordersid) async {
    data.clear();
    statusRequest = StatusRequest.loading;
    var response = await pendingOrdersData
        .approveOrders(
          myServices.sharedPreferences.getInt('id')!,userid,ordersid
        );
    print('==========$response');
    statusRequest = handlingData(response);
    if (response['status'] == 'success') {
